...
function getData(saveId, readOrWrite, randMode, groupSize, text) {
		...
		$.ajax({
		    ...
		    success : function(response) {
		    	...
		    	if (readOrWrite == 'write') { //if this function is used to write
		    		...
		    		writeData();  //only run this once the getData() AJAX is finished
		    	}
...
function writeData() {	
	...
	$.ajax({
	    ...
	    data : {
	    json : JSON.stringify(existingData)  //When sending data to a web server, the data has to be a string.
	    }  //this replaces whatever's in the file with the updated existingData
	});
...
function clearData() {  //erases everything from JSON file!
